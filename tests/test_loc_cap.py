...
from strict_module.loc_cap import (
    count_lines,
    load_baseline,
    find_python_files,
    generate_baseline,
...
class TestLoadBaseline:
    ...
    def test_load_baseline_missing_file(self, tmp_path):
        ...
        baseline = load_baseline(str(tmp_path / "nonexistent.txt"))
        assert baseline == {}
...
class TestFindPythonFiles:
    ...
    def test_find_python_files_nonexistent_path(self):
        ...
        files = find_python_files("/nonexistent/path")
        assert files == {}
...
class TestGenerateBaseline:
    ...
    def test_generate_baseline_basic(self, tmp_path):
        ...
        output = generate_baseline(str(tmp_path))
    ...
    def test_generate_baseline_with_floor(self, tmp_path):
        ...
        (tmp_path / "file1.py").write_text("a\n" * 600)
        (tmp_path / "file2.py").write_text("b\n" * 100)
        output = generate_baseline(str(tmp_path), floor=200)
...
class TestRunLocCap:
    ...
    def test_run_loc_cap_all_pass(self, tmp_path):
        ...
        (tmp_path / "file1.py").write_text("a\n" * 100)
        (tmp_path / "file2.py").write_text("b\n" * 200)
        exit_code = run_loc_cap(str(tmp_path), hard_cap=694, soft_target=500)
        assert exit_code == 0
    ...
    def test_run_loc_cap_generate_baseline(self, tmp_path, capsys):
        ...
        (tmp_path / "file1.py").write_text("a\n" * 100)
        (tmp_path / "file2.py").write_text("b\n" * 50)
        exit_code = run_loc_cap(str(tmp_path), generate=True)
        assert exit_code == 0
        captured = capsys.readouterr()
    ...
    def test_run_loc_cap_custom_hard_cap(self, tmp_path):
        ...
        (tmp_path / "file.py").write_text("a\n" * 200)
        exit_code = run_loc_cap(str(tmp_path), hard_cap=150, soft_target=100)
        assert exit_code == 1
